...
class SBEND: public Element
{
	...
	int Nint;
	 ...
	 SBEND(double l=0, double angle=0, double e1=0, double e2=0, int N_kicks=1)
	{
		L=l;
		ANGLE=angle;
        		E1=e1;
...
class QUAD: public Element
{
 public:
    double K1L,K1SL;
     int Nint;
    QUAD(double l=0, double k1=0, double tilt=0, int N_kicks=1 )
    	{
		L=l;
		K1L=k1*l*cos(2*tilt);
...
class SEXT: public Element
{
 public:
    double K2L,K2SL;
     int Nint;
    SEXT(double l=0, double k2=0, double tilt=0, int N_kicks=1 )
    	{
		L=l;
		K2L=k2*l*cos(3*tilt);
...
class CMB_SBEND: public Element
{
 public:
   ...
   int Nint,Norder;
    CMB_SBEND( double l=0, double angle=0, double e1=0, double e2=0, int N_kicks=1, double kn[11]=NULL)
    { 
      int i;
...
{
 public:
  RFCAV( double l, double vrf, double frf, double phase0)
    { 
...
template <class T> 
void RFCAV_Pass(T x[6], RFCAV CA)
{
	T u,L;
//  DRIFT_Pass(x, L/2.);
    L=CA.L/2;
...
template <class T>
void bend_mult_kick_pass(T x[6], double L, double href, int Norder, double KNL[11])
{
  int i;
  int fac=1;
  T   Xn,  Yn, Xn0, Yn0;
...
template <class T> 
double CMB_SBEND_Pass(T x[6], CMB_SBEND C) 
{

       int i;
	double sum=0,num,totn=0;
...
radiation(x, C.L/C.ANGLE,  _CFkick1*Lint, Lint/3);
...
// radiation(x[6], CMB_SBEND B, double y)
radiation(x, C.L/C.ANGLE,  _CFkick2*Lint, Lint/3);
