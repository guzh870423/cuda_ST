...
//	 DRIFT_Pass(x,Fdrift1*Lint);
	...
	if( Es1>i * Lint ||  Es2>Lint* (0.6666666666f + C.Nint -1 - i) )
	{
		...
		{
			radiation(x, href, Lint*(i + 0.333333333f)-Es1, Lint*(i + 0.333333333f)-Es1, s);
		}

		if(Es2>Lint* (0.6666666666f + C.Nint -1 - i ) && Es2<Lint * (1+C.Nint -1 - i))
		{
			radiation(x, href, Lint* (1+C.Nint -1 - i)-Es2, Lint* (1+C.Nint -1 - i)-Es2, s);
		}
	}
	else
	{
		radiation(x, href, Lint/3, Lint/3, s);
	}
...
//	 DRIFT_Pass(x,Fdrift2*Lint);
	...
	if( Es1> Lint * (i+0.33333333f) ||  Es2>Lint* (0.33333333f + C.Nint -1 - i ) )
	{
		...
		{
			radiation(x, href, Lint*(i + 0.666666666f)-Es1, Lint*(i + 0.666666666f)-Es1, s);
		}

		if(Es2>Lint* (0.33333333f + C.Nint -1 - i) && Es2<Lint * (0.666666666f+C.Nint -1 - i) )
		{
			radiation(x, href, Lint* (0.666666666f+C.Nint -1 - i)-Es2, Lint* (0.666666666f+C.Nint -1 - i)-Es2, s);
		}
	}
	else
	{
...
//	 DRIFT_Pass(x,Fdrift2*Lint);
	...
	if( Es1> Lint * (i+0.666666666f) ||  Es2>Lint* ( C.Nint -1 - i ) )
	{
		...
		{
			radiation(x, href, Lint*(i + 1)-Es1, Lint*(i + 1)-Es1, s);
		}

		if(Es2>Lint* (C.Nint -1 - i) && Es2<Lint * (0.33333333f+C.Nint -1 - i) )
		{
			radiation(x, href, Lint* (0.33333333f+C.Nint -1 - i)-Es2, Lint* (0.33333333f+C.Nint -1 - i)-Es2, s);
		}
	}
	else
	{
...
//Edge angle 2
	...
	if(Es2<0)
	{
		radiation(x, href, -Es2, -Es2, s);	
	} 

} 

template <class T> 
void __device__ radiation(T x[6], T href, T L, T dsISR, curandState *s)
{
	T f, xp, yp,ddel,dsFactor, rand_num;
	f = (1+x[0]*href)/sqrt((1+x[5])*(1+x[5])-x[1]*x[1]-x[3]*x[3]);
	xp = x[1]*f;
 	yp = x[3]*f;
	dsFactor =  sqrt( (1+x[0]*href)*(1+x[0]*href) + xp*xp + yp*yp);


//average relative energy loss
	ddel=_beta*_beta*_beta * C_gamma /2/3.1415926 *E0*E0*E0 * (1+x[5])*(1+x[5]) *href*href * L * dsFactor;

//gaussian rms 
//	do {rand_num=curand_normal(s);} while(abs(rand_num)>3);
rand_num=curand_normal(s);
	ddel += sqrt( 0.315865513*C_gamma*hbar*v_c*_gamma*_gamma*_gamma *E0*E0*(href*href*href)/_beta*dsISR*dsFactor) * (1+x[5])*(1+x[5]) * rand_num;
